...
const aboutUs = () => {
  const founders = Founders.map((Founder) => {
    return (
    <div key={Founder.name} className='glass shadow-[rgba(255,255,255,0.50)] rounded-xl w-72 h-96 p-3'>
      <Image
        src={`/image/${Founder.photo}`}
        width='30'
        height='30'
        alt=''
      />
  ...
  const developers = Developers.map((Developer) => {
    return (
    <div key={Developer.name} className='glass shadow-[rgba(255,255,255,0.50)] rounded-xl w-56 h-75 p-3'>
      <Image
  ...
  const teamLeaders = TeamLeaders.map((TeamLeader) => {
    return (
    <div key={TeamLeader.name} className='grid-cols-5 glass shadow-[rgba(255,255,255,0.50)] rounded-xl  h-75 p-3'>
      <Image
  ...
  const memberList = Members.map((member) => {
    return (
    <div key={member.name} className='grid-cols-5 glass shadow-[rgba(255,255,255,0.50)] rounded-xl  h-75 p-3'>
      <Image
  ...
  return (
    <div>
      <Layout className='text-white w-screen py-8 flex flex-col gap-10 md:gap-16 items-center overflow-x-hidden'>
        ...
        <div className='flex justify-center grid-cols-5 w-1/4 md:w-full md:p-8 gap-4 md:gap-10 text-center px-auto'>
          {teamLeaders}
        </div>
